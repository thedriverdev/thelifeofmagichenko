...
let bulletPriceDisplay = document
 .getElementById("bulletPriceDisplay");
const bulletPrice = 10;
bulletPriceDisplay.innerHTML =
 "Bullet price: ₦" + bulletPrice +
 "/bullet.";
let firingRateDisplay = document
 .getElementById(
  "firingrate");
let interval;
...
function shooting() {
 ...
 if (firingRate <= 19) {
  ...
  interval = setInterval(decrement, 113);
  ...
  firingRateDisplay.style.color =
   "red";
  firing.innerHTML =
   "Firing: Gun overheating!";
  firingRateDisplay.innerHTML =
   "Firing rate: Gun overheating!";
  clearInterval(interval);
  interval = setInterval(decrement, 105);
 }
}
